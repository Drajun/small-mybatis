package cn.drajun.mybatis.datasource.pooled;


import cn.drajun.mybatis.datasource.unpooled.UnpooledDataSource;
...
public class PooledDataSource implements DataSource {
    ...
    // 空闲连接的存活时间
    protected int poolMaximumCheckoutTime = 25000;
    // 等待空闲的连接时间，这是给连接池一个打印日志状态机会的低层次设置,还有重新尝试获得连接, 这些情况下往往需要很长时间(为了避免连接池没有配置时静默失败)。
    protected int poolTimeToWait = 20000;
    // 用来用来验证连接是否正常工作的SQL语句,并且准备接受请求。默认是“NO PING QUERY SET” ,这会引起许多数据库驱动连接由一个错误信息而导致失败
    protected String poolPingQuery = "NO PING QUERY SET";
    // 开启或禁用侦测查询
    protected boolean poolPingEnabled = false;
    // 用来配置poolPingQuery多长时间用一次
                    ...
                    if(state.activeConnections.size() < poolMaximumActiveConnections){
                        ...
                        // 等待空闲的连接
                            ...
                            try {
                                ...
                                logger.info(Thread.currentThread().getName() + " waiting as long as " + poolTimeToWait + " milliseconds for connection.");
            ...
            if(poolPingEnabled){
                if(poolPingConnectionsNotUsedFor >= 0 && conn.getTimeElapsedSinceLastUse() > poolPingConnectionsNotUsedFor){
                    try{
                        ...
                        result = true;
                        logger.info("Connection " + conn.getRealHashCode() + " is normal!");
                    }
                    catch (Exception e){
